...
export default function Button({
  ...
  const glow = variant === "primary" ? "hover:shadow-[0_0_16px_rgba(126,200,227,0.35)] dark:hover:shadow-[0_0_20px_rgba(126,200,227,0.25)] transition-shadow duration-300" : "";

  return (
    <button
      {...props}
      className={`inline-flex items-center justify-center gap-2 rounded-lg font-medium transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed ${variants[variant]} ${sizes[size]} ${fullWidth ? "w-full" : ""} ${glow} ${variant === "primary" ? "active:scale-[0.97]" : ""} ${className}`}
    >
      {leftIcon}
      {children}
      {rightIcon}
