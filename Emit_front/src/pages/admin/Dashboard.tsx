...
import { conflits, statutNiveaux } from "@/data/mock";
import { useData } from "@/context/DataContext";

export default function AdminDashboard() {
  const { salles, journal, enseignants, cours } = useData();
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard label="Enseignants" value={enseignants.length} icon={<Users className="h-5 w-5" />} />
        <StatCard label="Salles" value={salles.length} icon={<Building2 className="h-5 w-5" />} />
        <StatCard label="Cours planifiés" value={cours.length} icon={<CalendarCheck className="h-5 w-5" />} />
        <StatCard label="Conflits actifs" value={conflits.length} tone="red" icon={<AlertTriangle className="h-5 w-5" />} />
      </div>
